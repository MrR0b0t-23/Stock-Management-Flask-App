...
if ENV == 'dev':
    ...
    app.debug= False
    app.config['SQLALCHEMY_DATABASE_URI']='postgres://cfiqxyjofltjmw:7923be22047e3ff8b889611dbe5c628b62f1a1cdc28c7ac2bfa82226bf7eb22a@ec2-52-72-221-20.compute-1.amazonaws.com:5432/d2vq668gqh8149'
...
class logs_data(db.Model):
    ...
    id= db.Column(db.Integer, primary_key = True )
    BlockName= db.Column(db.String, nullable= False)
    Contactor= db.Column(db.String, nullable= False)
    PhoneNo= db.Column(db.Integer, nullable=False)
    Scheme= db.Column(db.String, nullable=False)
    Place= db.Column(db.String, nullable=False)
    NameOfWork= db.Column(db.String, nullable= False)
...
class transfer_data (db.Model):
    ...
    id= db.Column(db.Integer, primary_key= True)
    BlockName= db.Column(db.String, nullable= False)
    FromScheme= db.Column(db.String, nullable=False)
    ToScheme= db.Column(db.String, nullable=False)
    Material= db.Column(db.String, nullable=False)
    Quantity= db.Column(db.Integer, nullable=False)
    Author= db.Column(db.String, nullable= False)
    DateTime= db.Column(db.DateTime, nullable=False)
...
class steel_data (db.Model):
    ...
    def __init__(self,BlockName, Scheme, Steel_8mm, Steel_10mm,Steel_12mm,Steel_16mm,Steel_20mm, DateTime):
        ...
        self.Steel_10mm= Steel_10mm
        self.Steel_12mm= Steel_12mm
        self.Steel_16mm= Steel_16mm
        self.Steel_20mm= Steel_20mm
        self.DateTime = DateTime
...
class other_data (db.Model):
    ...
    def __init__(self,BlockName, Scheme, Door, Window_1, Window_2, Toilet_Door, LogoTiles, DateTime):
        ...
        self.Toilet_Door= Toilet_Door
        self.LogoTiles= LogoTiles
        self.DateTime = DateTime
    ...
    if request.method=='POST':
        ...
        if user :
            ...
            if password== 'd!cDC:YKnr=b956':
                ...
                resp.set_cookie('AccessId','SuperAdmin')
                return resp
            else :
                result= 'none'
                return render_template('login.html', result=result)    
